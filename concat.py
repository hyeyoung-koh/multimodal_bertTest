...
for i in tqdm(range(0,10)): #인덱스가 0부터 64977이다.
    myfile='clip_'+str(i)
...
type(list_last_hidden_states[1][0][0]) #torch.Tensor 가 type이다.
type(mfcc_extracted) #이게 type이 numpy.ndarray이다.
mfcc_extracted=torch.from_numpy(mfcc_extracted)
print(mfcc_extracted)
print(mfcc_extracted.shape) #shape을 출력하면 torch.Size([10903,130])이다.
#numpy ndarray를 tensor로 변환하려면
print(list_last_hidden_states[1][0][0].shape) #출력하면 torch.Size([768])이다.
#다 1차원으로 변환하자.
mfcc_extracted_flatten=mfcc_extracted.flatten()
print(mfcc_extracted_flatten.shape) #torch.Size([141739]) 이다.

for i in tqdm(range(0,10)):
    #pooled_output=list_last_hidden_states[i][0][0]
    pooled_output=torch.cat([list_last_hidden_states[i][0][0],mfcc_extracted_flatten],dim=0)
    #pooled_output=torch.cat([list_last_hidden_states[i][0][0],mfcc_extracted],dim=2)
    #concat 텐서 합치기
    ...
    #stack함수를 이용한다면
    #pooled_output=torch.stack([list_last_hidden_states[i][0][0],mfcc_extracted_flatten],dim=1)
    ...
    #labels=torch.tensor(labels)
    logits=classifier(pooled_output) #여기서 multiply하는 과정에서 오류 발생
