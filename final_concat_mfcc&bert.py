...
def calculate_nfft(samplerate, winlen):
    window_length_samples = winlen * samplerate #winlen=frame_length이다.
    nfft = 1
    while nfft < window_length_samples:
        nfft *= 2
    return nfft
#nfft는 frame의 length를 결정하는 파라미터.n_fft의 기본값은 20이다. 더 다양한 데이터 특징을 추출하기 위해서->100까지 증가시킨 예시가 존재함.
...
def mfcc_extraction(dir_path,audio_format='wav'):
    ...
    if audio_format=='wav':
        ...
        for aud_file in files:
            ...
            mfcc_feature=mfcc(sig,rate) #여기서 mfcc함수를 사용함.
            return mfcc_feature
            cntr+=1
...
#mfcc 시각화
#librosa.display.specshow(feat, sr=16000, x_axis='time')
